...
@section('content')
    <style>
        ...
        * {
        ...
        .sales-list ol.alternating-colors {
                                            ...
                                            @if ($customerInfo && $customerInfo->band_id && $customerInfo->band_id > 0 && $customerInfo->band_id != '')
                                                <div class="col-md-4">
                                                    ...
                                                    <input type="text" class="form-control float-right"
                                                        id="current_balance" placeholder="0.00"
                                                        value="{{ $band->current_balance }}" readonly>
                                                </div>
                                                <div class="col-md-4">
                                                    <label for="">Withdraw Amount</label>
                                                    <input type="text" class="form-control" placeholder="0.00"
                                                        id="withdraw_amount" oninput="setValuetoWithdraw()"
                                                        id="withdraw_amount">
                                                    <small><span class="text-danger"
                                                            id="withdraw_amountP_error"></span></small>
                                                </div>
                                                <div class="col-md-4">
                                                    <label for="">Left amount</label>
                                                    <input type="text" class="form-control float-right" id="left_amount"
                                                        placeholder="0.00" value="{{ $band->current_balance }}" readonly>
                                                </div>
                                                <div class="col-md-12">
                                                    <hr>
                                                    <button type="submit"
                                                        id="save_withraw"class="btn btn-primary">Withdraw</button>
                                                </div>
                                            @endif
                                        </form>
                                    </div>
    ...
    <script>
        function setValuetoWithdraw() {
            ...
            if (withdraw_amount > current_balance) {
                $('#withdraw_amountP_error').text('You can not withdraw more than your current balance');
                document.getElementById('withdraw_amount').value = '';
                return;
            } else {
                $('#withdraw_amountP_error').text('');
            }
            var left_amount = current_balance - withdraw_amount;
            document.getElementById('left_amount').value = left_amount;
        }
        $('#save_withraw').click(function(e) {
            e.preventDefault(); // Prevent default form submission

            var withdraw_amount = parseFloat(document.getElementById('withdraw_amount').value);
            var current_balance = parseFloat(document.getElementById('current_balance').value);

            if(isNaN(withdraw_amount) || withdraw_amount == '' || withdraw_amount < 1){
                $('#withdraw_amountP_error').text('Please enter valid amount');
                document.getElementById('withdraw_amount').value = '';
                return;
            }
            if (withdraw_amount > current_balance) {
            ...
            $.ajax({
                url: "{{ route('accountBandBillWithrawStore') }}",
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    ...
                    if (response == 'success') {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                window.location.href = "{{ url('account_sales') }}";
                            } else if (result.isDenied) {
                                window.location.href = "{{ url('account_band_bill_withraw_view') }}";
                            }
                        });
