...
def main(user_id, product_id, action):
    try:
        ...
        with open("confusion_matrix.txt", "w") as file:
            ...
            file.write(str(cm))
            logging.info(cm)
        ##logging.info("Confusion matrix saved to confusion_matrix.txt.")

        # Step 6: Save recommendations to the database
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ##logging.error("Invalid arguments. Usage: python rec.py <user_id> <product_id> <action>")
        sys.exit(1)
